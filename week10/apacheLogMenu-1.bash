...
function frequentVisitors() {
	...
	local visitorIps=$(cat "$logFile" | cut -d' ' -f1 | sort | uniq -c | awk '$1 > $10 {print}')
	echo "$visitorIps"
}
...
function suspiciousVisitors() {
	...
	visitList=$(cat "ioc.txt")
	grep -f <(echo "$visitList") "LogFile" | cut -d' ' -f1 | sort | uniq -c
...
do
	...
	echo "[3] Display only Pgaes"
	echo "[4] Histogram"

	echo "[5] Display Frequent visitors"

	echo "[6] Display suspicius visitors"
	echo "[7] Quit"
